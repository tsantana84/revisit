...
must_haves:
  truths:
    ...
    - "Owner settings page can read and write branding fields (primary_color, secondary_color, logo_url, program_name, earn_rate, reward_type, point_expiry_days) without schema errors"
    - "Ranks can be saved with multiplier and min_visits values without schema errors"
    - "Owner can upload a logo image to storage and retrieve its public URL"
  artifacts:
    - path: "supabase/migrations/0005_branding.sql"
      provides: "Branding and config columns on restaurants (primary_color, secondary_color, logo_url, program_name, earn_rate, reward_type, point_expiry_days) + multiplier/min_visits on ranks"
      contains: "ALTER TABLE public.restaurants"
    - path: "supabase/migrations/0006_storage.sql"
      provides: "restaurant-logos storage bucket with INSERT/DELETE RLS policies"
...
<task type="auto">
  ...
  <action>
    ...
    **src/app/dashboard/owner/layout.tsx** — Server Component. Calls `supabase.auth.getUser()`. If not authenticated, redirect to `/login`. Then call `supabase.auth.getSession()` and decode JWT to verify `app_role === 'owner'`. If not owner, redirect to `/login`. This is a defense-in-depth check (middleware does fast redirect, layout does authoritative check). Render children with basic dashboard shell: header with restaurant name placeholder, logout button calling the logout action, and a sidebar/nav with links to `/dashboard/owner` ("Painel"), `/dashboard/owner/team` ("Equipe"), and `/dashboard/owner/settings` ("Configuracoes"). These nav links are created here so that Plans 02-02 and 02-03 do NOT need to modify this layout file.
