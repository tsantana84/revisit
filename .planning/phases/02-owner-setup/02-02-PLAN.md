...
<task type="auto">
  ...
  <action>
    ...
    Add navigation link to the team page from the owner dashboard layout or the owner dashboard page. The simplest approach: add a nav link in `src/app/dashboard/owner/layout.tsx` sidebar/nav that links to `/dashboard/owner/team`.
  </action>
  <verify>
    - `npx tsc --noEmit` passes
