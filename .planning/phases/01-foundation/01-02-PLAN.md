...
must_haves:
  truths:
    ...
    - "CI fails if any public table is missing RLS — enforced by a test querying pg_class for relrowsecurity=false"
  artifacts:
...
7. **"Every public table has RLS enabled"** — Using the service_role client, call `supabase.rpc('check_rls_enabled')` or query `pg_class` directly via a raw SQL RPC. The test should query: `SELECT c.relname FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'public' AND c.relkind = 'r' AND c.relrowsecurity = false AND c.relname NOT LIKE 'schema_%'`. Assert the result is empty (zero tables without RLS). If any table lacks RLS, fail with a message listing the unprotected tables. This test acts as the CI enforcement mechanism — since `npm run test:rls` runs in CI, any new table added without RLS will break the build.
...
IMPORTANT: Each test case must create its OWN Supabase client with ANON_KEY and sign in — do NOT reuse clients across tests. The JWT is baked into the client after signIn.
  ...
  <verify>
Run `supabase start` (if not running), then `npm run test:rls`. All 7 test cases must pass. The output should show:
...
- All public tables: RLS enabled (zero unprotected)
  </verify>
  <done>All 7 cross-tenant isolation tests pass using the Supabase JS SDK against a local Supabase instance. Tests verify data isolation, unauthenticated access denial, cross-tenant write prevention, JWT claim injection, and that every public table has RLS enabled (CI enforcement).</done>
</task>
...
3. `npm run test:rls` passes all 7 test cases
...
- CI enforcement: a test asserts every public table has relrowsecurity=true — adding a table without RLS breaks the build
</success_criteria>
