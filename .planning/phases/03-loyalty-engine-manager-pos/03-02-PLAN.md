...
must_haves:
  truths:
    ...
    - "RANK-05 wallet card color change is deferred to Phase 4 — this plan returns rank_promoted and new_rank_name in SaleState for Phase 4 consumption but does not implement card color logic"
  artifacts:
...
Note: SaleState includes `rankPromoted` and `newRankName` fields. These satisfy RANK-05 partially — the RPC returns promotion info which the UI displays as a notice. Wallet card color change based on rank is deferred to Phase 4 (per research: "color change is a display concern handled by Phase 4"). This plan ensures the data (`rank_promoted`, `new_rank_name`) is available for Phase 4 consumption.
...
Use proper pt-BR characters (accents) in all user-facing strings. Do NOT use the Zod `error` shorthand for refine — use `{ message: '...' }` syntax for refine validators.
  ...
  <done>pos.ts exports lookupCustomer and registerSale Server Actions with proper auth, validation, points calculation, and RPC integration. Both use discriminated union state types compatible with useActionState. SaleState includes rankPromoted and newRankName for Phase 4 wallet card color consumption.</done>
</task>
...
6. SaleState returns rank_promoted and new_rank_name for downstream use
</verification>
...
- Rank promotion data (rank_promoted, new_rank_name) is surfaced in SaleState for Phase 4 wallet card color
</success_criteria>
