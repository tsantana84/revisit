...
<task type="auto">
  <name>Task 2: Add discount_pct field to RanksForm</name>
  <files>src/app/dashboard/owner/settings/RanksForm.tsx</files>
  <action>
...
Wait — `src/lib/actions/restaurant.ts` is also modified by this task. Add it to the files list.
  </action>
  <verify>Run the dev server (`npm run dev`), navigate to /dashboard/owner/settings, verify the discount_pct field appears in each rank row. Saving ranks should persist the discount_pct value.</verify>
