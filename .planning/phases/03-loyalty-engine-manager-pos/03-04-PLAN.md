...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app/dashboard/manager/page.tsx"
      ...
      via: "import registerRedemption and checkRewardAvailability for reward display"
      pattern: "import.*registerRedemption"
    - from: "src/app/dashboard/manager/page.tsx"
      to: "src/lib/actions/rewards.ts#checkRewardAvailability"
      via: "RewardSection wrapper calls checkRewardAvailability via server action to display reward info after sale success"
      pattern: "checkRewardAvailability"
---
...
- `registerRedemption`, `checkRewardAvailability` from `@/lib/actions/rewards`
...
**Reward display via RewardSection wrapper:**
After a successful sale, render a `RewardSection` inline component (defined in the same file or as a nested component). The RewardSection:
1. On mount (or on button click "Verificar Recompensa"), calls `checkRewardAvailability` as a server action with the customer's card_number and restaurantId
2. Displays the returned `RewardInfo`:
   - cashback: "Credito disponivel: R$ {availableCredit}"
   - free_product: "Recompensa disponivel: {rewardName}" with "Resgatar" button (if available)
   - progressive_discount: "Desconto de {discountPct}% ({rankName})" — informational only, no redemption button needed
   - none: nothing shown
3. If a redemption button is shown, it submits a form bound to `registerRedemption` useActionState with hidden fields: card_number, reward_config_id, reward_type
4. On redemption success, show confirmation message

This is the single committed approach — no alternatives. The RewardSection wrapper handles the async call to checkRewardAvailability and renders reward state.
...
**MGR-07 compliance:** The page renders ONLY the card lookup form, sale confirmation, and reward redemption. There are NO links, navigation items, or routes to analytics, customer lists, settings, or any other section. The layout.tsx sidebar already has minimal nav — verify it only shows "Painel" and "Sair" (no additional links).
  ...
  <done>Manager POS page renders the full card lookup -> sale confirmation -> success flow with RewardSection wrapper calling checkRewardAvailability for reward display. Only card lookup and sale registration UI is accessible — no analytics, customer list, or configuration navigation.</done>
</task>
...
What was built: Complete Manager POS flow with card lookup and check-digit validation, two-step sale confirmation with points preview, success screen with rank promotion notice, RewardSection wrapper calling checkRewardAvailability for reward display, and manager layout stripped to single-function UI.
...
4. At /dashboard/manager, verify:
   ...
   g. After sale success, reward info should be displayed via RewardSection
   h. Click "Nova Venda" — should return to card lookup
...
6. Try navigating to /dashboard/owner — should be blocked by middleware
  ...
  <done>Manager POS flow works end-to-end: lookup, preview, confirm, success, reward display. Layout is single-function. Type "approved" or describe issues.</done>
</task>
...
5. RewardSection displays reward availability after sale success via checkRewardAvailability
6. Manager layout has no navigation to analytics, customer list, or settings
7. Invalid card numbers are rejected before any database query
</verification>
...
- Reward availability displayed via RewardSection wrapper using checkRewardAvailability
