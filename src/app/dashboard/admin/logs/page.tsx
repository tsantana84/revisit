...
export default async function AdminLogsPage({ searchParams }: Props) {
  ...
  const buildTabUrl = (t: string, p?: string) => {
    ...
    return `/dashboard/admin/logs?${params.toString()}`
  }
  ...
  if (safeTab === 'vendas') {
    let salesQuery = supabase
      .from('active_sales')
      .select('id, amount_cents, points_earned, created_at, customer_id, staff_id, restaurant_id', {
        count: 'exact',
      })
  ...
  if (safeTab === 'atividade') {
    let auditQuery = supabase
      ...
      .select(
        'id, customer_id, points_delta, balance_after, transaction_type, note, reference_id, created_at, restaurant_id',
        { count: 'exact' }
      )
      .order('created_at', { ascending: false })
      .range(from, to)
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-db-border">
                  {['Data/Hora', 'Restaurante', 'Cliente', 'Cartão', 'Valor', 'Pontos', 'Registrado por'].map((h) => (
                    <th
                      key={h}
                  ...
                  salesRows.map((sale) => {
                    ...
                    return (
                      <tr key={sale.id} className="border-b border-db-border last:border-b-0 transition-colors hover:bg-white/[0.02]">
                        ...
                        <td className="px-4 py-3 text-db-text-secondary text-xs">
                          {restaurantNameMap.get(sale.restaurant_id) ?? '—'}
                        </td>
        ...
        <div className="db-card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-db-border">
                  {['Data/Hora', 'Restaurante', 'Cliente', 'Tipo', 'Pontos', 'Saldo', 'Gerente', 'Nota'].map((h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 text-xs font-medium text-db-text-muted uppercase tracking-wider whitespace-nowrap"
                    >
                  ...
                  auditRows.map((tx) => {
                    ...
                    return (
                      <tr key={tx.id} className="border-b border-db-border last:border-b-0 transition-colors hover:bg-white/[0.02]">
                        ...
                        <td className="px-4 py-3 text-db-text-secondary text-xs">
                          {restaurantNameMap.get(tx.restaurant_id) ?? '—'}
                        </td>
                        <td className="px-4 py-3 text-db-text-secondary">
                          {customer?.name ?? '—'}
                        </td>
